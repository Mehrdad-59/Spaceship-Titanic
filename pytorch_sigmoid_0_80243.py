# -*- coding: utf-8 -*-
"""Spaceship Titanic _PyTorch_Sigmoid_0.80243.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1bL1UN8igvkqVx5XagmRFkJnU8VNitGDa
"""
...
class TitanicModel(nn.Module):
  def __init__(self):
    ...
    self.batch2=nn.BatchNorm1d(150)
...
y_pred=torch.tensor(np.zeros((X_test.shape[0],n_splits))).cuda()
y_pred_fold_Thr=torch.tensor(np.zeros((X.shape[0],n_splits))).cuda()
y=y.cpu()
...
for fold, (tr_idx, te_idx) in enumerate(skf.split(X,y)):
  ...
  with torch.no_grad() :
    X=X.cuda()
    X_test=X_test.cuda()
    y_pred_fold_Thr[:, fold]=gpumodel(X).data.squeeze()
    y_pred_fold=gpumodel(X)
    y_pred[:, fold]+=gpumodel(X_test).data.squeeze()
    y=y.cpu()
    y_pred_fold=y_pred_fold.cpu()
    y_pred_fold_final=torch.where(y_pred_fold<0.5,0,1)
  print(f'fold {fold+1} Accuracy Score: {accuracy_score(y,y_pred_fold_final): 8.6f}')
print(f'\nDuration:{time.time() - start_time:.0f} seconds')

y_pred=y_pred.cpu().detach().numpy()
y_pred_fold_Thr=y_pred_fold_Thr.cpu().detach().numpy()
y=y.cpu().detach().numpy()

y_pred=gmean(y_pred, axis=1)

y_pred_fold_Thr=gmean( y_pred_fold_Thr, axis=1)

np.save('preds_pytorch.npy', y_pred)
np.save('val_preds_pytorch.npy',y_pred_fold_Thr )
...
scores = [f1_score(y, to_labels(y_pred_fold_Thr, t)) for t in thresholds]
ix = np.argmax(scores)
print('Threshold=%.3f, F-Score=%.5f' % (thresholds[ix], scores[ix]))

y_pred_final=np.where(y_pred<thresholds[ix],0,1)
